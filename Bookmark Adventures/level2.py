...
# Title & Icon
pygame.display.set_caption('Bookmark Adventures - Jungle')
...
class BlockJungle(pygame.sprite.Sprite):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("images/BlockWoods.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]

class PoisonBlock(pygame.sprite.Sprite):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("images/Poison.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]
...
class MushroomBlock(pygame.sprite.Sprite):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
...
class WeakBlock(pygame.sprite.Sprite):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("images/Weak.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]
...
class Tom(pygame.sprite.Sprite):    
    ...
    def collide(self, movx, movy, world):
        ...
        for a in weakBlocksList:
            if self.rect.colliderect(a):
                self.weakness -= 1
                if self.weakness == 0:
                    weakBlocksList.remove(a)
                if self.weakness == 0:
                    self.weakness = 20

        for d in mushroomBlocksList:
            if self.rect.colliderect(d):
                self.health -= 15    
                mushroomBlocksList.remove(d)
                pygame.mixer.Channel(2).play(pygame.mixer.Sound('music/vomit.ogg'))
            if self.health <= 0:
                pygame.mixer.music.set_volume(0)
                pygame.mixer.Channel(1).play(pygame.mixer.Sound('music/lose.ogg'))
                Tom.Lose()
...
class Level(object):
    def __init__(self, open_level):
        ...
        self.mushroomBlocksList = pygame.sprite.Group()
        self.winBlocksList = pygame.sprite.Group()
        self.weakBlocksList = pygame.sprite.Group()
    ...
    def create_level(self, x, y):
        ...
        for row in self.level2:
            for col in row:
                ...
                # Woods Block Obstacle
                if col == "X":
                    blockJungle = BlockJungle(x, y)
                    self.world.append(blockJungle)
                    self.all_sprite.add(self.world)                                                  
                # Poison Block Obstacle
                if col == "F":
                    poisonBlock = PoisonBlock(x, y)
                    self.badBlocks.append(poisonBlock)
                ...
                # Weak Block
                if col == "A":
                    weakBlock = WeakBlock(x, y)
                    self.weakBlocks.append(weakBlock)
                    self.weakBlocksList.add(self.weakBlocks)
                # Mushroom Block
                if col == "D":
                    mushroomBlock = MushroomBlock(x, y)
                    self.mushroomBlocks.append(mushroomBlock)
                    self.mushroomBlocksList.add(self.mushroomBlocks)
...
background = pygame.image.load("images/Forest Background.png").convert_alpha()
...
mushroomBlocks = level.mushroomBlocks
weakBlocks = level.weakBlocks
...
mushroomBlocksList = level.mushroomBlocksList
weakBlocksList = level.weakBlocksList
...
def startLevel2():
    ...
    # Música del nivel 
    pygame.mixer.music.load("music/jungle.ogg")  
    ...
    while True:
        ...
        camera.draw_sprites(screen, mushroomBlocksList)
        camera.draw_sprites(screen, weakBlocksList)
